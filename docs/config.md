...
| numbering_sequence| dict              | Can be used to add systematic with similar name, differing just by the number of the nuisance parameter  (see description bellow)|
...
| **Option**        | **Value type**    | **Function** |
| ----------------- | ----------------- | ------------ |
| replacement_string| string            | If the string is found in ```up``` or ```down``` options for the systematic variations, it will be replaced by the number. This will add new systematics for each value of the replacement string (see lines bellow) |
